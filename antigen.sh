...
rm -f Makefile.in aclocal.m4 configure config.h.in depcomp install-sh missing compile config.guess config.sub ltmain.sh config.log configure.scan autoscan.log
find . -name \*~ -exec rm -f {} \;
